...
#define AI_NET_OBJ_INSTANCE g_sine_test
 
#undef AI_SINE_TEST_MODEL_SIGNATURE
#define AI_SINE_TEST_MODEL_SIGNATURE     "94c5b139a8074b53c31108ef2df03270"
...
#define AI_TOOLS_DATE_TIME   "Mon Mar 15 12:06:54 2021"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_SINE_TEST_N_BATCHES
#define AI_SINE_TEST_N_BATCHES         (1)
...
AI_STATIC ai_array serving_default_dense_9_input0_output_array;   /* Array #6 */
...
AI_STATIC ai_tensor serving_default_dense_9_input0_output;   /* Tensor #6 */
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 128, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SINE_TEST_IN_NUM, &serving_default_dense_9_input0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SINE_TEST_OUT_NUM, &dense_2_output),
  &dense_0_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool sine_test_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, AI_SINE_TEST_ACTIVATIONS_ALIGNMENT));
  AI_ASSERT(activations)
  AI_UNUSED(net_ctx)

  {
    /* Updating activations (byte) offsets */
    serving_default_dense_9_input0_output_array.data = AI_PTR(NULL);
    serving_default_dense_9_input0_output_array.data_start = AI_PTR(NULL);
...
AI_DECLARE_STATIC
ai_bool sine_test_configure_weights(
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_SINE_TEST_MODEL_NAME,
      .model_signature   = AI_SINE_TEST_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_sine_test_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_sine_test_create(
...
AI_API_ENTRY
ai_handle ai_sine_test_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_sine_test_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= sine_test_configure_weights(net_ctx, &params->params);
  ok &= sine_test_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_sine_test_forward(ai_handle network, const ai_buffer* input)
{
