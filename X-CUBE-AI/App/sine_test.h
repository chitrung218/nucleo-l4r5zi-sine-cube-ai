...
#ifndef __AI_SINE_TEST_H__
#define __AI_SINE_TEST_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_SINE_TEST_MODEL_NAME          "sine_test"

#define AI_SINE_TEST_ACTIVATIONS_ALIGNMENT   (4)

#define AI_SINE_TEST_IN_NUM       (1)
#define AI_SINE_TEST_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_SINE_TEST_IN_SIZE { \
  (1 * 1 * 1), \
}
#define AI_SINE_TEST_IN_1_SIZE  (1 * 1 * 1)
#define AI_SINE_TEST_IN_1_SIZE_BYTES  ((1 * 1 * 1) * 4)





#define AI_SINE_TEST_OUT_NUM      (1)
#define AI_SINE_TEST_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_SINE_TEST_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_SINE_TEST_OUT_1_SIZE  (1 * 1 * 1)
#define AI_SINE_TEST_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)

#define AI_SINE_TEST_N_NODES (5)
 ...
 * @ref ai_sine_test_create. Both the activations memory buffer 
 ...
 * using @ref ai_sine_test_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_SINE_TEST_*
 * defines into file @ref sine_test.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_sine_test_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_SINE_TEST_H__*/
